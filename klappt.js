...
// end include: /tmp/tmp39ncuf1o.js
// include: /tmp/tmpfvocz921.js

    // All the pre-js content up to here must remain later on, we need to run
    // it.
    if ((typeof ENVIRONMENT_IS_WASM_WORKER != 'undefined' && ENVIRONMENT_IS_WASM_WORKER) || (typeof ENVIRONMENT_IS_PTHREAD != 'undefined' && ENVIRONMENT_IS_PTHREAD) || (typeof ENVIRONMENT_IS_AUDIO_WORKLET != 'undefined' && ENVIRONMENT_IS_AUDIO_WORKLET)) Module['preRun'] = [];
    var necessaryPreJSTasks = Module['preRun'].slice();
  // end include: /tmp/tmpfvocz921.js
...
var ASM_CONSTS = {
  713627: () => { if (typeof navigator == 'undefined' || typeof window == 'undefined') { return 0; } const ua = navigator.userAgent || ""; const platform = navigator.platform || ""; const mobilePattern = new RegExp('Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini', 'i'); const mobileUA = mobilePattern.test(ua); const ipadDesktopMode = platform == 'MacIntel' && (navigator.maxTouchPoints || 0) > 1; const coarsePointer = window.matchMedia && window.matchMedia('(pointer: coarse)').matches; return (mobileUA || ipadDesktopMode || coarsePointer) ? 1 : 0; },  
 714181: () => { const state = Module.lexiSDLTextInput; if (!state || !state.input) { return; } state.submitted = false; state.blurred = false; state.active = false; try { state.input.blur(); } catch (e) { } state.input.style.display = 'none'; },  
 714412: ($0, $1, $2, $3, $4, $5) => { let state = Module.lexiSDLTextInput; if (!state) { state = {}; state.input = null; state.submitted = false; state.blurred = false; state.active = false; Module.lexiSDLTextInput = state; } let input = state.input; if (!input) { input = document.createElement('input'); input.id = '__lexi_sdl_text_input'; input.autocomplete = 'off'; input.style.position = 'fixed'; input.style.left = '0px'; input.style.top = '0px'; input.style.width = '1px'; input.style.height = '1px'; input.style.opacity = '0'; input.style.pointerEvents = 'none'; input.style.border = '0'; input.style.padding = '0'; input.style.margin = '0'; input.style.background = 'transparent'; input.style.color = 'transparent'; input.style.caretColor = 'transparent'; input.style.outline = 'none'; input.style.zIndex = '2147483647'; input.style.fontSize = '16px'; input.addEventListener( 'input', function() { _mobile_text_input_web_wakeup(); }); input.addEventListener( 'keydown', function(event) { if (event.key == 'Enter') { state.submitted = true; _mobile_text_input_web_wakeup(); event.preventDefault(); input.blur(); } }); input.addEventListener( 'blur', function() { if (state.active) { state.blurred = true; } state.active = false; _mobile_text_input_web_wakeup(); }); document.body.appendChild(input); state.input = input; } input.style.display = 'block'; input.type = UTF8ToString($1); input.inputMode = UTF8ToString($2); input.autocapitalize = UTF8ToString($3); input.autocorrect = $4 ? 'on' : 'off'; input.spellcheck = !!$4; input.dir = $5 ? 'rtl' : 'ltr'; input.style.textAlign = $5 ? 'right' : 'left'; input.value = UTF8ToString($0); state.submitted = false; state.blurred = false; state.active = true; try { input.focus({preventScroll : true}); const end = input.value.length; input.setSelectionRange(end, end); } catch (e) { } },  
 716218: ($0, $1, $2, $3) => { const state = Module.lexiSDLTextInput; if (!state || !state.input) { return; } state.input.style.left = $0 + 'px'; state.input.style.top = $1 + 'px'; state.input.style.width = $2 + 'px'; state.input.style.height = $3 + 'px'; },  
 716447: ($0, $1) => { const state = Module.lexiSDLTextInput; if (!state || !state.input) { return; } stringToUTF8(state.input.value || "", $0, $1); },  
 716577: () => { const state = Module.lexiSDLTextInput; if (!state) { return 0; } const submitted = state.submitted ? 1 : 0; state.submitted = false; return submitted; },  
 716732: () => { const state = Module.lexiSDLTextInput; if (!state) { return 0; } const blurred = state.blurred ? 1 : 0; state.blurred = false; return blurred; },  
 716879: () => { if (typeof(Module['SDL3']) === 'undefined') { Module['SDL3'] = {}; } var SDL3 = Module['SDL3']; if (typeof(SDL3.JSVarToCPtr) === 'undefined') { SDL3.JSVarToCPtr = function(v) { return v; }; } if (typeof(SDL3.CPtrToHeap32Index) === 'undefined') { SDL3.CPtrToHeap32Index = function(ptr) { return ptr >>> 2; }; } },  
 717193: ($0) => { var str = UTF8ToString($0) + '\n\n' + 'Abort/Retry/Ignore/AlwaysIgnore? [ariA] :'; var reply = window.prompt(str, "i"); if (reply === null) { reply = "i"; } return reply.length === 1 ? reply.charCodeAt(0) : -1; },  
 717408: () => { Module['SDL3'].dummy_audio = {}; Module['SDL3'].dummy_audio.timers = []; Module['SDL3'].dummy_audio.timers[0] = undefined; Module['SDL3'].dummy_audio.timers[1] = undefined; },  
 717585: ($0, $1, $2, $3, $4) => { var a = Module['SDL3'].dummy_audio; if (a.timers[$0] !== undefined) { clearInterval(a.timers[$0]); } a.timers[$0] = setInterval(function() { dynCall('vi', $3, [$4]); }, ($1 / $2) * 1000); },  
 717777: ($0) => { var a = Module['SDL3'].dummy_audio; if (a.timers[$0] !== undefined) { clearInterval(a.timers[$0]); } a.timers[$0] = undefined; },  
 717908: ($0) => { var parms = new URLSearchParams(window.location.search); for (const [key, value] of parms) { if (key.startsWith("SDL_")) { var ckey = stringToNewUTF8(key); var cvalue = stringToNewUTF8(value); if ((ckey != 0) && (cvalue != 0)) { dynCall('iiii', $0, [ckey, cvalue, 1]); } _Emscripten_force_free(ckey); _Emscripten_force_free(cvalue); } } },  
 718249: () => { if (typeof(AudioContext) !== 'undefined') { return true; } else if (typeof(webkitAudioContext) !== 'undefined') { return true; } return false; },  
 718396: () => { if ((typeof(navigator.mediaDevices) !== 'undefined') && (typeof(navigator.mediaDevices.getUserMedia) !== 'undefined')) { return true; } else if (typeof(navigator.webkitGetUserMedia) !== 'undefined') { return true; } return false; },  
 718630: () => { var SDL3 = Module['SDL3']; if (typeof(SDL3.audio_playback) === 'undefined') { SDL3.audio_playback = {}; } if (typeof(SDL3.audio_recording) === 'undefined') { SDL3.audio_recording = {}; } if (!SDL3.audioContext) { if (typeof(AudioContext) !== 'undefined') { SDL3.audioContext = new AudioContext(); } else if (typeof(webkitAudioContext) !== 'undefined') { SDL3.audioContext = new webkitAudioContext(); } if (SDL3.audioContext) { if ((typeof navigator.userActivation) === 'undefined') { autoResumeAudioContext(SDL3.audioContext); } } } return (SDL3.audioContext !== undefined); },  
 719209: () => { return Module['SDL3'].audioContext.sampleRate; },  
 ...
 744372: ($0) => { var dialog_id = UTF8ToString($0); var dialog = document.getElementById(dialog_id); if (!dialog) { return false; } return dialog.open; },  
 744510: ($0) => { var dialog_id = UTF8ToString($0); var dialog = document.getElementById(dialog_id); if (!dialog) { return 0; } try { return parseInt(dialog.returnValue); } catch(e) { return 0; } },  
 744692: ($0, $1) => { alert(UTF8ToString($0) + "\n\n" + UTF8ToString($1)); }
};
